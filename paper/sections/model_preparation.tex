...
上述方程构成各问的共同基础。\textbf{第一问}在固定半径、常热物性下计算 $0$--$1800\,\mathrm{s}$ 的预热阶段；\textbf{第二问}从 $t=0$ 重新引入附录~3 的状态相关物性，并在附件~1 的连续分段线性边界下计算前三小时，不把 $1800\,\mathrm{s}$ 作为人为阶段切换；\textbf{第三问}在明确附件末端之后的边界延拓后，利用第二问保存的全网格状态判定首次全部达标时刻；\textbf{第四问}进一步引入尺寸收缩，调整输运方程、计算区域和表面条件。各问继承同一守恒框架，具体物性、边界延拓与区域条件分别确定。
