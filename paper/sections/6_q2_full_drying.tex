...
为检验问题二所建模型的可靠性，令其物性关系退化为问题一的参数形式，而初边值条件与求解方法均保持不变。退化后，两问对应同一传热传质模型。由表~\ref{tab:q2-validation} 可知，当 $N=1280$ 时，两种模型在整个计算域内的最大温度差和最大含水率差分别为 $1.32\times10^{-7}\,{}^\circ\mathrm{C}$ 和 $1.82\times10^{-9}\,\mathrm{kg/kg}$。这说明在问题一的参数条件下，问题二模型能够回归基础模型，离散方法与边界条件的处理具有一致性。

再对水分扩散过程进行积分平衡检验。由 $C=\mathrm e^z$ 还原干基含水率后，分别计算药材平均含水率的减少量和经表面散失的累计含水率，理论上二者应当相等。基准计算的最大残差为 $1.12\times10^{-11}\,\mathrm{kg/kg}$，时间加密后进一步降至 $1.49\times10^{-12}\,\mathrm{kg/kg}$。作为对照，若按 $1\,\mathrm{s}$ 间隔用梯形公式累计表面通量，最大残差为 $5.04\times10^{-7}\,\mathrm{kg/kg}$；采用高阶时间积分后，残差降至 $10^{-11}$ 量级，表明前者的误差主要来自表面通量的累计精度。

\input{tables/q2_validation}

综合以上结果，{\heiti\bfseries 退化模型与问题一的基础模型相符，域内含水率的减少量也与表面流出量保持一致，说明问题二所得数值解具有较好的可靠性。}这里的平衡建立在干基含水率的有效扩散假设上，并不等同于真实多相介质中的水质量守恒。
